...
let register = class register {
    constructor(formBuiler, router, loginService, registerService) {
    ...
    onSubmit() {
        ...
        this.registerService.register(this.registerForm.value)
            ...
            this.loginService.login(this.registerForm.controls.email.value, this.registerForm.controls.password.value)
                .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["first"])())
                .subscribe(data => {
                this.router.navigate(['/mainmenu']);
            }, error => {
...
let specsDistribution = class specsDistribution {
    constructor() {
    ...
    distributeSpecs() {
        this.apiService.getPoints(this.day, this.specs).subscribe(data => {
            if (data['status'] == 'ok') {
...
let kanbantable = class kanbantable {
    constructor(apiService, loginService, router) {
        this.apiService = apiService;
        this.loginService = loginService;
        this.router = router;
            ...
            for (let j = 0; j < this.CardList[i].length; j++) {
                if (this.CardList[i][j].idCard == $event) {
                    ...
                    switch (i) {
                        case 1: {
                            department = 'anal';
                            progress = this.CardList[i][j].CurrentAnalysis;
                            break;
                        }
                        case 3: {
                            department = 'dev';
                            progress = this.CardList[i][j].CurrentDevelopment;
                            break;
                        }
                        case 5: {
                            department = 'test';
                            progress = this.CardList[i][j].CurrentTesting;
                            break;
                        }
                    }
                    this.apiService.updateCard($event, department, progress)
                        .subscribe(data => {
                        let c = this.CardList[i][j];
                        c.updateStatus();
            ...
            if (this.expedice[i][0] != null) {
                if (this.expedice[i][0].idCard == $event) {
                    ...
                    switch (i) {
                        case 1: {
                            department = 'anal';
                            progress = this.expedice[i][0].CurrentAnalysis;
                            break;
                        }
                        case 3: {
                            department = 'dev';
                            progress = this.expedice[i][0].CurrentDevelopment;
                            break;
                        }
                        case 5: {
                            department = 'test';
                            progress = this.expedice[i][0].CurrentTesting;
                            break;
                        }
                    }
                    this.apiService.updateCard($event, department, progress)
                        .subscribe(data => {
                        let Card = this.expedice[i][0];
                        Card.updateStatus();
                        if (i == 6) {
...
let LoginService = class LoginService {
    ...
    login(email, password) {
        return this.http.post('http://controller-getkanban.apps.us-west-1.starter.openshift-online.com/controller/api/sign_in', { email, password })
